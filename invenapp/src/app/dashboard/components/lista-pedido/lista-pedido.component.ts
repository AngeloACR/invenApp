...
export class ListaPedidoComponent implements OnInit {
  ...
  async generatePdf(event, i) {
    const documentDefinition = await this.getDocumentDefinition(i);
    pdfMake.createPdf(documentDefinition).open();
  }

  getDocHeader(company, logo, correlativo, fecha, vendedor, condicionVenta) {
    let headerLeft = [
      {
        text: company.name,
        style: "name"
